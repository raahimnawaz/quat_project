...
static void on_timer_tick(void *user_data) {
  ...
  const float dt = 0.001f; // 1 kHz simulation step

  // --- 1. RK4 Integration for Angular Velocity (w) ---
  ...
  // --- 2. Exact Exponential Map for Quaternion (q) ---
  float w_norm = sqrt(chip->w[0]*chip->w[0] + chip->w[1]*chip->w[1] + chip->w[2]*chip->w[2]);
  float theta = w_norm * dt / 2.0f;
  float dq[4];
  
  if (theta < 1e-8f) { // Small-angle approximation to avoid division by zero
    dq[0] = 1.0f;
  ...
  // q_new = q \otimes dq (Hamilton product)
  ...
  // Occasional re-normalization to mop up float roundoff error
  static int step = 0;
  if (++step >= 100) {
    step = 0;
...
static uint8_t on_i2c_read(void *user_data) {
  ...
  // Direct memory mapping for float arrays (Little-Endian transmission)
...
void chip_init(void) {
  ...
  chip->q[0] = 1.0f; // Initial identity quaternion

  // M1 Dzhanibekov test conditions:
  chip->J[0] = 1.0f; // Asymmetric inertia
  chip->J[1] = 2.0f; 
  chip->J[2] = 3.0f; 
  
  chip->w[0] = 0.01f; // Tiny perturbation on X
  chip->w[1] = 5.0f;  // Fast spin on intermediate axis (Y)
  chip->w[2] = 0.0f;
